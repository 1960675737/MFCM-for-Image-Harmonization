...
class HDataset(BaseHDataset):
    def __init__(self, dataset_path, split, blur_target=False, **kwargs):
        ...
        images_lists_paths = [x for x in self.dataset_path.glob('all*.txt') if x.stem.endswith(split)]
        # images_lists_paths = [x for x in self.dataset_path.glob('sub*.txt') if x.stem.endswith(split)]
