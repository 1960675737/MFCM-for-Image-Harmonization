...
class SeparableConv2d(nn.Module):                                                          # Depthwise Separable Convolution
...
class SeparableConv2d_X(nn.Module):                                                          # Depthwise Separable Convolution       modified by XCJ
    def __init__(self, in_channels, out_channels, kernel_size=4, padding=1, stride=2,
                 activation=nn.LeakyReLU(0.2, True), use_bias=True, norm_layer=nn.BatchNorm2d):
        super(SeparableConv2d_X, self).__init__()
        self.body = nn.Sequential(
