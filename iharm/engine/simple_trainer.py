...
from iharm.model.losses import Regularization
...
class SimpleHTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        self.teacher_device = cfg.teacher_device
        self.net = model

        # student network training : loading teacher network
        if self.cfg.model_type == 'student':
            self.teacher_net = teacher_model
            self._load_teacher_weights()
            if cfg.multi_gpu:
                self.teacher_net = _CustomDP(self.teacher_net, device_ids=cfg.gpu_ids, output_device=cfg.gpu_ids[0])
            self.teacher_net = self.teacher_net.to(self.device)

        self._load_weights() ################################################################ need load pretrained teacher network weights
        if cfg.multi_gpu:
            self.net = _CustomDP(self.net, device_ids=cfg.gpu_ids, output_device=cfg.gpu_ids[0])
        self.net = self.net.to(self.device)
        self.lr = optimizer_params['lr']

        self.reg_weight_decay = self.cfg.reg_weight_decay  # regularization weight decay parameter
        if self.reg_weight_decay > 0:
            self.reg_loss = Regularization(self.net, self.reg_weight_decay, p=2)
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.reg_weight_decay > 0:
                loss = loss + self.reg_loss.forward()              # loss = loss + self.reg_loss(self.net)
    ...
    def validation(self, epoch):
        ...
        if self.min_val_loss > np.array(losses_logging['pixel_loss']).mean():
            self.min_val_loss = np.array(losses_logging['pixel_loss']).mean()
            self.best_model_epoch = epoch
            save_checkpoint(self.net, self.cfg.CHECKPOINTS_PATH, prefix='best',
                            epoch=None, multi_gpu=self.cfg.multi_gpu)  # save best model according to val FN-MSE loss
            logger.info(
                f'The current best model is saved. The corresponding epoch is {self.best_model_epoch}, validation loss: {self.min_val_loss:.6f}')
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            if self.cfg.model_type == 'teacher':
                images, masks = batch_data['target_images'], batch_data['masks']
            else:
            ...
            if self.cfg.model_type == 'student':
                teacher_output = self.teacher_net(batch_data['target_images'], masks)

            loss = 0.0
            loss = self.add_loss('pixel_loss', loss, losses_logging, validation, output, batch_data)

            if self.cfg.use_ssim_loss:
                loss = self.add_loss('ssim_loss', loss, losses_logging, validation, output, batch_data)

            if self.cfg.use_ssim_focal_loss:
                loss = self.add_loss('ssim_focal_loss', loss, losses_logging, validation, output, batch_data)

            if self.cfg.use_brightness_loss:
                loss = self.add_loss('brightness_loss', loss, losses_logging, validation, output, batch_data)

            if self.cfg.model_type == 'teacher':
                loss = self.add_loss('imitation_loss', loss, losses_logging, validation, output, batch_data)
            elif self.cfg.model_type == 'student':
                loss = self.add_loss('distill_loss', loss, losses_logging, validation, output, batch_data, teacher_outputs=teacher_output)

            if self.cfg.use_vgg_loss:
                loss = self.add_loss('vgg_loss', loss, losses_logging, validation, output, batch_data)
    ...
    def add_loss(self, loss_name, total_loss, losses_logging, validation, net_outputs, batch_data, teacher_outputs=None):
        loss_cfg = self.loss_cfg if not validation else self.val_loss_cfg
        loss_weight = loss_cfg.get(loss_name + '_weight', 0.0)
        if loss_weight > 0.0:
            loss_criterion = loss_cfg.get(loss_name)

            if loss_name == 'pixel_loss' :
            ...
            elif loss_name =='brightness_loss' :
                loss = loss_criterion(*(net_outputs.get(x) for x in loss_criterion.pred_outputs),
                                      *(batch_data[x] for x in loss_criterion.gt_outputs))

            elif loss_name == 'ssim_loss':
                loss = loss_criterion(net_outputs.get('images') * torch.sigmoid(3.0 * batch_data['masks']),
                                      batch_data['target_images'] * torch.sigmoid(3.0 * batch_data['masks']))

            elif loss_name == 'ssim_focal_loss':
                loss = loss_criterion(net_outputs.get('images') * torch.sigmoid(3.0 * batch_data['masks']),
                                      batch_data['target_images'] * torch.sigmoid(3.0 * batch_data['masks']))

            elif loss_name == 'imitation_loss':
                loss = loss_criterion(net_outputs.get('mid-images'),
                                      batch_data['images'])

            elif loss_name == 'distill_loss':
                loss = loss_criterion(net_outputs.get('distill_results'), teacher_outputs.get('distill_results'))

            elif loss_name == 'vgg_loss' :                                                       # modified by XCj
                loss = loss_criterion(net_outputs.get('images'), batch_data['target_images'])
    ...
    def _load_teacher_weights(self):
        if self.cfg.teacher_weights is not None:
            if os.path.isfile(self.cfg.teacher_weights):
                load_weights(self.teacher_net, self.cfg.teacher_weights, verbose=True)
                self.cfg.teacher_weights = None
            else:
                raise RuntimeError(f"=> no checkpoint found at '{self.cfg.weights}'")
