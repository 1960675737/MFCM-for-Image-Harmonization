import argparse
import torch
import os
import os.path as osp
from pathlib import Path
import sys

import cv2
import numpy as np
import torch
from torchvision import transforms
from tqdm import tqdm

sys.path.insert(0, '.')
from iharm.inference.predictor import Predictor
from iharm.inference.utils import load_model, find_checkpoint
from iharm.mconfigs import ALL_MCONFIGS
from iharm.utils.log import logger
from iharm.utils.exp import load_config_file

from iharm.model import losses

# improved_dih256 /home/ivp_01/AnacondaProject/image_harmonization/harmonization_exps/fixed256/improved_dih/last_checkpoint.pth --images /home/ivp_01/AnacondaProject/image_harmonization/images/composite_images --masks /home/ivp_01/AnacondaProject/image_harmonization/images/masks --real_images /home/ivp_01/AnacondaProject/image_harmonization/images/real_images --resize 256

def main():
    args, cfg = parse_args()

    device = torch.device(f'cuda:{args.gpu}')
    checkpoint_path = find_checkpoint(cfg.MODELS_PATH, args.checkpoint)
    net = load_model(args.model_type, checkpoint_path, verbose=True)
    predictor = Predictor(net, device)

    image_names = os.listdir(args.images)
    # with open(args.images_lists,'r') as f:
    #     image_names = [x.strip() for x in f.readlines()]

    def _save_image(image_name, bgr_image):
        # rgb_image = cv2.cvtColor(bgr_image, cv2.COLOR_RGB2BGR)
        rgb_image = bgr_image
        cv2.imwrite(
            str(cfg.RESULTS_PATH / f'{image_name}'),
            rgb_image,
            [cv2.IMWRITE_JPEG_QUALITY, 85]
        )

    logger.info(f'Save images to {cfg.RESULTS_PATH}')

    resize_shape = (args.resize, ) * 2

    txtfile = './images/loss_analyze.txt'
    f = open(txtfile, 'a+')

    # for image_name in tqdm(image_names):
    for image_name in image_names:

        image_path = osp.join(args.images, image_name)
        image = cv2.imread(image_path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image_size = image.shape
        if resize_shape[0] > 0:
            image = cv2.resize(image, resize_shape, cv2.INTER_LINEAR)

        mask_path = osp.join(args.masks, '_'.join(image_name.split('_')[:-1]) + '.png')
        mask_image = cv2.imread(mask_path)
        if resize_shape[0] > 0:
            mask_image = cv2.resize(mask_image, resize_shape, cv2.INTER_LINEAR)
        mask = mask_image[:, :, 0]
        # mask[mask <= 100] = 0
        # mask[mask > 100] = 1
        mask = mask.astype(np.float32) / 255.

        object_mask = mask_image.astype(np.float32)

        real_image_path = osp.join(args.real_images, '_'.join(image_name.split('_')[:-2]) + '.jpg')
        real_image = cv2.imread(real_image_path)
        real_image = cv2.cvtColor(real_image, cv2.COLOR_BGR2RGB)
        if resize_shape[0] > 0:
            real_image = cv2.resize(real_image, resize_shape, cv2.INTER_LINEAR)

        # trans = transforms.ToTensor()
        # image = trans(image)
        # real_image = trans(real_image)

        output = predictor.predict(image, mask)

        if args.original_size:
            output = cv2.resize(output, image_size[:-1][::-1])

        viz_image = np.hstack((image, object_mask, real_image, output)).astype(np.uint8)
        _save_image(image_name, viz_image[:, :, ::-1])

        image = image.astype(np.float32)
        image = image.transpose((2, 0, 1))
        image = torch.tensor(image)

        real_image = real_image.astype(np.float32)
        real_image = real_image.transpose((2,0,1))
        real_image = torch.tensor(real_image)

        output = output.transpose((2, 0, 1))
        output = torch.tensor(output)

        mask = torch.tensor(mask)
        mask = torch.unsqueeze(mask, dim=0)

        pixel_loss_criterion = losses.MaskWeightedMSE(min_area=100)
        loss_0 = pixel_loss_criterion(image, real_image, mask)
        loss_0 = torch.mean(loss_0)
        loss_1 = pixel_loss_criterion(output, real_image, mask)
        loss_1 = torch.mean(loss_1)

        vgg_loss_criterion = losses.VGGLoss(device)

        image = torch.unsqueeze(image, dim=0).cuda()
        real_image = torch.unsqueeze(real_image, dim=0).cuda()
        output = torch.unsqueeze(output, dim=0).cuda()

        vgg_loss_0 = vgg_loss_criterion(image, real_image)
        vgg_loss_0 = torch.mean(vgg_loss_0)
        vgg_loss_1 = vgg_loss_criterion(output, real_image)
        vgg_loss_1 = torch.mean(vgg_loss_1)

        print(image_name, 'FN-MSE of  comp and real is :', loss_0, '; pred and real is :', loss_1)
        message = 'FN-MSE Loss : %s comp and real : %f ; pred and real : %f \n'% (image_name, loss_0, loss_1)
        f.write(message)
        print(image_name, 'VGG Loss of  comp and real is :', vgg_loss_0, '; pred and real is :', vgg_loss_1)
        message = 'VGG Loss : %s comp and real : %f ; pred and real : %f \n' % (image_name, vgg_loss_0, vgg_loss_1)
        f.write(message)
    f.close()


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('model_type', choices=ALL_MCONFIGS.keys())
    parser.add_argument('checkpoint', type=str,
                        help='The path to the checkpoint. '
                             'This can be a relative path (relative to cfg.MODELS_PATH) '
                             'or an absolute path. The file extension can be omitted.')

    parser.add_argument(
        '--images_lists', type=str,
        help='Path to images split txt file.'
    )
    parser.add_argument(
        '--images', type=str,
        help='Path to directory with .jpg images to get predictions for.'
    )
    parser.add_argument(
        '--masks', type=str,
        help='Path to directory with .png binary masks for images, named exactly like images without last _postfix.'
    )
    parser.add_argument(
        '--real_images', type=str,
        help='Path to directory with .jpg real images, named exactly like images without last _postfix.'
    )
    parser.add_argument(
        '--resize', type=int, default=256,
        help='Resize image to a given size before feeding it into the network. If -1 the network input is not resized.'
    )
    parser.add_argument(
        '--original-size', action='store_true', default=False,
        help='Resize predicted image back to the original size.'
    )
    parser.add_argument('--gpu', type=str, default=0, help='ID of used GPU.')
    parser.add_argument('--config-path', type=str, default='./config.yml', help='The path to the config file.')
    parser.add_argument(
        '--results-path', type=str, default='',
        help='The path to the harmonized images. Default path: cfg.EXPS_PATH/predictions.'
    )

    args = parser.parse_args()
    cfg = load_config_file(args.config_path, return_edict=True)
    cfg.EXPS_PATH = Path(cfg.EXPS_PATH)
    cfg.RESULTS_PATH = Path(args.results_path) if len(args.results_path) else cfg.EXPS_PATH / 'predictions'
    cfg.RESULTS_PATH.mkdir(parents=True, exist_ok=True)
    logger.info(cfg)
    return args, cfg


if __name__ == '__main__':
    main()
